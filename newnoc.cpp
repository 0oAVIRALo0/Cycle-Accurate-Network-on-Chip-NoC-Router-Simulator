...
void addToMap5(int &startcycle,string state,double delay,string flitBinary,string flitType,string start,string status){
    if(delay==cycleTime){
          logmap[startcycle].push_back({start,state,flitBinary,flitType,status});
          startcycle++;
    }
        ...
        logmap[startcycle].push_back({start,state,flitBinary,flitType,status+newextra});
        startcycle++;
    }
}
...
vector<Port*> sendPacketXYRouting(string start,string destination,Router *Router1,int startcycle){
    ...
    addToMap5(startcycle,"localbuffer",currRouter->parentRouter->IOdelay,flitBinary,flitType,start,extra);
    //------------------decoding 
    congestionCheck(start,startcycle,"SA");
    addToMap5(startcycle,"SA",currRouter->parentRouter->switchAllocatorDelay,flitBinary,flitType,start," ");
    ...
    addToMap5(startcycle,"XBAR",currRouter->parentRouter->xbarDelay,flitBinary,flitType,start," ");
    // data transfer XBAR----------------------------------
    ...
    while(true){
        ...
        if((startcycle-prevCycle)!=0){
            extra+=("delayed by "+to_string(startcycle-prevCycle)+" cause : congestion");}
        addToMap5(startcycle,nextOppPort->direction+"buffer",currRouter->router->IOdelay,flitBinary,flitType,currRouter->router->id,extra);
        // ---------------------buffer stage---------------------------------
        ...
        if(currRouter->router->id==destination){
            ...
            startcycle++;
            logmap[startcycle].push_back({(currRouter->router->id),"XBAR",flitBinary,flitType,"flit removed to local PE"});
            break;
        }
        extra="";
        prevCycle=startcycle;
        ...
        addToMap5(startcycle,"SA",currRouter->router->switchAllocatorDelay,flitBinary,flitType,currRouter->router->id,extra);
        oppPort=getOppositePort(nextRouter);    //copy data from input to direction output
        currRouter->output=flitBinary;
        oppPort->input=flitBinary;
        addToMap5(startcycle,"XBAR",currRouter->router->xbarDelay,flitBinary,flitType,currRouter->router->id," ");
...
void printLogMap(map<int, vector<vector<string>>>& logmap,int clk) {
    ...
    for (auto& data : logmap) {
        for (auto& vec : data.second) {
            logFile<<left<<setw(15)<<data.first<<setw(15)<<data.second.size()<<setw(20)<<vec[0]<<setw(20)<<vec[1]<<setw(40)<<vec[2]<<setw(15)<<vec[3]<<setw(15)<<vec[4]<<endl;
            // if(vec)
        }
    }
}
...
void printReport_pvs(map<int, vector<vector<string>>>& logmap,int clk) {
    ...
    for (auto& data : logmap) {
        for (auto& vec : data.second) {
            reportFile_pvs<<left<<setw(15)<<vec[0]<<setw(15)<<vec[1]<<setw(35)<<vec[2]<<clk*(data.first-(flitStart[vec[2]]))<<endl;
        }
    }
}
